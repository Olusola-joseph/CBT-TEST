...
class CBTExamApp {
    ...
    async loadQuestionsForSubject(subject) {
        try {
            // Load the selected subject and year to the database using the new method
            if (examDB && examDB.db) {
                await examDB.loadSubjectToDatabase(subject, this.selectedYear);
                console.log(`Database updated with ${subject} questions for ${this.selectedYear}`);
            }
            
            // Load from the selected year's JSON file to populate the app's questions array
